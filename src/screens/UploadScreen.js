...
import { FlatList, StyleSheet, Dimensions, View, Text, ImageBackground } from 'react-native';
...
const UploadScreen = () => {
    const { state, chooseFile, deleteUploadImg, loading, reset } = useContext(Context);
    const LogProgress = () => {
        ...
        if (state.uploadFile.length > 0) {
            return (
                <FlatList
                    keyExtractor={uploadFile => uploadFile.id}
    ...
    const predict = async () => {
        for (let i = 0; i < state.uploadFile.length; i++) {
            ...
            try {
                let resp = await fetch('https://ae337200bd48.ngrok.io/predict', reqOption).then(data => data.blob());
    ...
    const deleteImages = async (...args) => {
        ...
        try {
            for (let i = 0; i < file.length; i++) {
                await fetch('/delete/' + file[i].file.name, reqOption).then(data => data.json());
